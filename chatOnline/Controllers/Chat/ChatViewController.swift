...
//  chatOnline
//
//  Created by Miguel Angel Saravia Belmonte on 10/6/20.
    ...
    @objc private func didTapComposeButton() {
        ...
        vc.completion = { [weak self] result in
            ...
            if let targetConversation = currentConversations.first(where: {
                ...
                let vc = MessageViewController(with: targetConversation.otherUserEmail, id: targetConversation.id)
    ...
    private func createNewConversation(result: SearchResult) {
        ...
        DatabaseManager.shared.conversationExists(iwth: email, completion: { [weak self] result in
            ...
            case .success(let conversationId):
                let vc = MessageViewController(with: email, id: conversationId)
                vc.isNewConversation = false
                vc.title = name
                vc.navigationItem.largeTitleDisplayMode = .never
                strongSelf.navigationController?.pushViewController(vc, animated: true)
            case .failure(_):
                let vc = MessageViewController(with: email, id: nil)
...
extension ChatViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func openConversation(_ model: Conversation) {
        let vc = MessageViewController(with: model.otherUserEmail, id: model.id)
        vc.title = model.name
        vc.navigationItem.largeTitleDisplayMode = .never
