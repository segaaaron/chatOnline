...
extension MessageViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let image = info[.editedImage] as? UIImage, let imageData =  image.pngData() {
            ...
            ManageFirebaseApi().uploadMessagePhoto(with: imageData, fileName: fileName, completion: { [weak self] result in
                guard let strongSelf = self else {
                    return
                }
                ...
                case .success(let urlString):
                    ...
                    guard let url = URL(string: urlString),
                        let placeholder = UIImage(named: "plus") else {
                            return
                    }
            ...
            ManageFirebaseApi().uploadMessageVideo(with: videoUrl, fileName: fileName, completion: { [weak self] result in
                guard let strongSelf = self else {
                    return
                }
